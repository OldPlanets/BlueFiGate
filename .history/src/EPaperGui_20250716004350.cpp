...
static constexpr float ICON_AREA_WIDTH = 30;
...
void EPaperGui::drawNewLayout()
{
    ...
    display->drawBitmap(display->width() - epd_icon_sun.width, 0, epd_icon_sun.data, epd_icon_sun.width, epd_icon_sun.height,
                        GxEPD_BLACK, GxEPD_WHITE);

    int valueAreaWidth = (display->width() - ICON_AREA_WIDTH) / 2; // Split remaining space
    int insideX = ICON_AREA_WIDTH;
    int outsideX = ICON_AREA_WIDTH + valueAreaWidth;
    ...
    // display->drawLine(ICON_AREA_WIDTH, 0, ICON_AREA_WIDTH, display->height(), GxEPD_BLACK);
    ...
    drawValueRow(epd_icon_thermometer, tempRowY,
                 ...
                 &FreeSans24pt7b, tempRowY - 10);

    // Humidity/Battery row
    int humidityRowY = tempRowY + 43; // Leave space for temperature
    ...
    {
        // Show battery row
        String insideValue = insideBattCritical ? String(batteryInside, 0) + "%" : formatHumidity(humidityInside);
        String outsideValue = outsideBattCritical ? String(batteryOutside, 0) + "%" : formatHumidity(humidityOutside);
...
void EPaperGui::drawValueRow(const Icon &icon, int y, const String &insideValue,
                             const String &outsideValue, const GFXfont *font, int iconY)
{
    int valueAreaWidth = (display->width() - ICON_AREA_WIDTH) / 2;
    int insideX = ICON_AREA_WIDTH;
    int outsideX = ICON_AREA_WIDTH + valueAreaWidth;

    // Draw icon centered in icon area
    int iconX = max((int)(ICON_AREA_WIDTH - icon.width) / 2, 0);
    iconY = iconY >= 0 ? iconY : y;
...
void EPaperGui::showLowPowerShutdown()
{
    initializeDisplay();
    ...
    do
    {
        ...
        // Draw battery icon centered on display
        int iconX = (display->width() - epd_icon_batteryempty.width) / 2;
        int iconY = (display->height() - epd_icon_batteryempty.height) / 2;

        display->drawBitmap(iconX, iconY, epd_icon_batteryempty.data,
