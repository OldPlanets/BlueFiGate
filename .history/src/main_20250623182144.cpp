...
#define bmx280.80_POWER GPIO_NUM_10
...
void ledFlashAsync(int duration)
{
    ...
    ledTimer.once_ms(duration, [](){ digitalWrite(ONBOARD_LED, HIGH); });
}
...
void setup()
{
    ...
    isUserInteraction = isWakeUpByUSB || (isExternPower && !isWakeUpByTimer);
    ...
    if (isUserInteraction)
    {
        ...
        if (wifiManager.requestWifi(&wifiManager, true))
        {
            ...
            ArduinoOTA.setHostname(OTAHOSTNAME).setPassword(OTAPASSWORD).setMdnsEnabled(true).onStart([]()
                                                                                                      {
                                String type;
    ...
    pinMode(bmx280.80_POWER, OUTPUT);
    digitalWrite(bmx280.80_POWER, HIGH);
    delay(50);
    Wire.begin(GPIO_NUM_20, GPIO_NUM_21);

    #ifdef USE_BME280
    #include <Adafruit_BME280.h>
        Adafruit_bmx280 bmx280.&Wire);
    bool bSensorReady = bmx280.begin(118);
    if (!bSensorReady)
    {
        Log.critical("BME280 error initalizing");
        Serial.print("SensorID was: 0x"); Serial.println(bmx280.sensorID(),16);
    }
    else
    {
    ...
    #else
    Adafruit_bmx280.80 bmx280(&Wire);
    bool bSensorReady = bmx280.begin(118);
    if (!bSensorReady)
    {
        Log.critical("BME280 error initalizing");
    ...
    digitalWrite(bmx280.80_POWER, LOW);
    pinMode(bmx280.80_POWER, INPUT);
