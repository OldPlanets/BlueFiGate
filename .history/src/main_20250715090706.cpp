...
const int USER_INTERACTION_STAYAWAKE_SEC = 120;
...
void setup()
{
    ...
    if (bSensorReady)
    {
        ...
        if (valid)
        {
            ...
            remoteStorage.updateValues(
                ...
                calculateLiFePO4SOC(batteryVoltage));
        }
        else
        {
    ...
    GxEPD2_BW<GxEPD2_213_BN, GxEPD2_213_BN::HEIGHT> display(GxEPD2_213_BN(DISPLAY_CS, DISPLAY_DC, DISPLAY_RST, DISPLAY_BUSY)); // DEPG0213BN 122x250, SSD1680
    SPI.begin(DISPLAY_SCL, -1 /* MISO not used */, DISPLAY_SDA, DISPLAY_CS);
    display.init(115200, true, 50, false);

    const char HelloWorld[] = "Hello Bla!";
    const char HelloWeACtStudio[] = "WeAct Studio";
    // Icon dimensions
    const int icon_width = 23;
    const int icon_height = 37;
    ...
    uint16_t icon_x = (display.width() - icon_width) / 2;
    uint16_t icon_y = 10; // Position icon near the top

    // Position text below the icon
    uint16_t text_x = ((display.width() - tbw) / 2) - tbx;
    uint16_t text_y = icon_y + icon_height + 20; // 20 pixels below icon
    ...
    do
    {
        ...
        // Draw the thermometer icon
        display.drawBitmap(icon_x, icon_y, epd_bitmap_thermometer, icon_width, icon_height, GxEPD_BLACK, GxEPD_WHITE);
        ...
        text_x = ((display.width() - tbw) / 2) - tbx;
        display.setCursor(text_x, text_y + tbh + 10); // 10 pixels below first text
...
{
    const bool canSleep = millis() > stayAwakeUntil && wifiManager.getWifiUserCount() == 0 && !bIsOtaRunning && !remoteStorage.isBusy();
    if (!canSleep)
    {
        yield();
        delay(50);
