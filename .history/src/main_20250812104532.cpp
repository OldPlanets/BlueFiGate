...
void loop()
{
    ...
    else if (bStartedUpSucceededNotification && !bIndicateError && (!WiFi.isConnected() || batteryManager.getLastTdtUpdateMs() > 1000 * 300))
    {
        ledColor(20, 0, 0, 0);
    }
    else if (bStartedUpSucceededNotification && bIndicateError && WiFi.isConnected() && batteryManager.getLastTdtUpdateMs() < 1000 * 30)
    {
        ledColor(0, 0, 0, 0);
    }

    if (batteryManager.getLastTdtUpdateMs() > 0 && batteryManager.getLastTdtUpdateMs() < 1000 * 600)
    {
        esp_restart();
    }
