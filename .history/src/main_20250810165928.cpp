...
const int USER_INTERACTION_STAYAWAKE_SEC = 5 * 60;
...
void deepSleepOrDelay()
{
    ...
    #endif
    #if USE_EINKDISPLAY
    canSleep = canSleep && !webRequestManager.isBusy() && temperatureManager.hasPolled() && bGuiUpdated;
    #endif
    if (!canSleep)
    {
        yield();
...
void loop()
{
    ...
    bReadyForPolling = bReadyForPolling && !remoteStorage.isBusy();
#endif
#if USE_EINKDISPLAY
    if (bReadyForPolling)
    {
        webRequestManager.process();
        if (temperatureManager.hasPolled() && !bGuiUpdated)
        {
            if (temperatureManager.hasValidResults())
            {
                gui.setBatteryOutside(temperatureManager.getBatteryOutside());
                gui.setTemperatureOutside(temperatureManager.getOutsideTemp());
                gui.setHumidityOutside(temperatureManager.getAltitude());
            }
            bGuiUpdated = true;
            gui.updateGui();
            gui.end();
