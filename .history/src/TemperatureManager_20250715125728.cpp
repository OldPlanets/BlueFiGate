...
TemperatureManager::TemperatureManager(PaperGui &gui, WebRequestManager &webRequestManager, Preferences &prefs)
    : m_gui(gui), m_webRequestManager(webRequestManager), m_prefs(prefs)
{
}
...
void TemperatureManager::loadFromPreferences()
{
}
    ...
    if (result.status == WebTaskStatus::SUCCESS)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                m_gui.setMainTempValue(m_outsideTemp, result.dataTimestamp);
                m_gui.setHumidityValue(m_batteryOutside);
