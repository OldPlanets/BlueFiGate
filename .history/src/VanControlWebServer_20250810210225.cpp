...
// Include your BatteryManager header here
// #include "BatteryManager.h"
...
VanControlWebServer::VanControlWebServer(int port, BatteryManager* batteryMgr) 
    : server(std::make_unique<AsyncWebServer>(port)), batteryManager(batteryMgr), isRunning(false) {
}
...
void VanControlWebServer::handleBatteryHtml(AsyncWebServerRequest* request) {
    ...
    const String html = generateBatteryHtml(data, timestamp);
    request->send(200, "text/html", html);
}

String VanControlWebServer::generateBatteryHtml(const TDTBMSData& data, time_t timestamp) const {
    String html = R"(
<!DOCTYPE html>
<html>
<head>
...
<body>
    ...
    if (data.batteryLevel < 20) socClass = "critical";
    else if (data.batteryLevel < 50) socClass = "warning";
    ...
    if (voltage < 12.0f) voltageClass = "critical";
    else if (voltage < 12.5f) voltageClass = "warning";
...
bool VanControlWebServer::start() {
    ...
    server->on("/battery.html", HTTP_GET, [this](AsyncWebServerRequest* request) {
        handleBatteryHtml(request);
    });
