...
void EPaperGui::drawSection(const char* title, float temp, float humidity, float battery, 
    ...
    // Available height: 122 pixels
    // Layout calculation:
    // - Title: FreeSans9pt7b ≈ 12px height + 15px from top = 27px
    // - Thermometer icon: 37px height
    // - Temperature text: FreeSans18pt7b ≈ 22px height + 8px spacing = 30px
    // - Humidity icon: 37px height  
    // - Humidity text: FreeSans12pt7b ≈ 16px height + 5px spacing = 21px
    // Total: 27 + 37 + 30 + 37 + 21 = 152px (too much!)
    
    // Optimized layout for 122px height:
    int currentY = y + 2; // Start with small margin
    
    // Title (12px + 3px margin = 15px total)
    display->setFont(&FreeSans9pt7b);
    drawCenteredText(title, x, currentY + 12, width, &FreeSans9pt7b);
    currentY += 15;
    
    // Temperature section - make this more compact
    // Thermometer icon (37px)
    ...
    currentY += epd_icon_thermometer.height + 2; // 39px total
    
    // Temperature text (22px + 3px spacing = 25px total)
    display->setFont(&FreeSans18pt7b);
    String tempStr = formatTemperature(temp);
    drawCenteredText(tempStr.c_str(), x, currentY + 18, width, &FreeSans18pt7b);
    currentY += 25;
    
    // Total so far: 15 + 39 + 25 = 79px
    // Remaining: 122 - 79 = 43px for humidity/battery section
    
    // Humidity/Battery section
