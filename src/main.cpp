...
const char *NVS_NAMESPACE = "blufigate";
...
TimeSync timeSync;
bool bCrashedBefore;
bool bStartedUpSucceededNotification = false;
bool bIndicateError = false;

bool bIsOtaRunning = false;

void ledColor(uint8_t red, uint8_t green, uint8_t blue, uint32_t durationMs = 0)
{
    pixel.setPixelColor(0, red, green, blue);
    pixel.show();
    if (durationMs > 0 && (red > 0 || green > 0 || blue > 0))
    {
        ledTimer.once_ms(durationMs, []() {
...
void loop()
{
    ...
    if (batteryManager.getLastTdtUpdateMs() > 0 && !bStartedUpSucceededNotification)
    {
        if (!bCrashedBefore)
        {
            bStartedUpSucceededNotification = true;
            ledColor(0, 20, 0, 1000 * 30);
        }
    }
    else if (bStartedUpSucceededNotification && !bIndicateError && (!WiFi.isConnected() || millis() - batteryManager.getLastTdtUpdateMs() > 1000 * 60))
    {
        bIndicateError = true;
        ledColor(20, 0, 0, 0);
    }
    else if (bStartedUpSucceededNotification && bIndicateError && WiFi.isConnected() &&  millis() - batteryManager.getLastTdtUpdateMs() < 1000 * 30)
    {
        bIndicateError = false;
        ledColor(0, 0, 0, 0);
    }

    if (batteryManager.getLastTdtUpdateMs() > 0 && millis() - batteryManager.getLastTdtUpdateMs() > 1000 * 600)
    {
        esp_restart();
    }
